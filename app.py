...
def main():
    ...
    if  option == "Trading Bot":
        ...
        st.text("")
        st.subheader("Top Premarket Gainers")
        st.dataframe(df_eligible_candidates)
        premarket_symbols =  df_eligible_candidates["Symbol"].tolist()

        #st.text("")
        #st.text("")
        #st.subheader("Top Gainers (WIP)")

        st.text("")
        st.text("")
        st.header("Gainers Pattern Recognition")


        #pattern = st.selectbox("Select Pattern", list(modules.patterns.keys()))
        ...
        for symbol in premarket_symbols:
            ...
            for key, value in modules.patterns.items():
                pattern_function = getattr(talib, value)
